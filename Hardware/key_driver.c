...
static void key_driver_scan(void *_scan_para)
{
    ...
    u8 key_event = 0;          // 存放待发送的按键事件
    u8 cur_key_value = NO_KEY; // 存放待发送的按键键值
    u8 key_value = 0;
    // struct sys_event e;

    u8 key_scan_status = KEY_SCAN_STATUS_NONE; // 状态机，负责控制该函数内的跳转操作
    ...
    if (cur_key_value != scan_para->last_key) // 如果当前键值与上一次扫描的键值不一样，说明有按键按下
    {
        /*
