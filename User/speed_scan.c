...
volatile u32 speed_pulse_cnt;
volatile u32 speed_scan_time_ms;
...
void update_cur_speed_scan_time(void)
{
...
void speed_scan(void)
{
    ...
    if (cur_speed_scan_time >= 500 || flag_is_speed_scan_over_time)
    {
        ...
        {
            const u32 tmp = (cur_speed_scan_pulse * SPEED_SCAN_MM_PER_TURN / SPEED_SCAN_PULSE_PER_TURN);
            cur_speed = tmp / 10 / cur_speed_scan_time;
            distance += tmp;
        }

        printf("cur speed %lu km/h\n", cur_speed);
        // printf("distance %lu m\n", distance / 100);
